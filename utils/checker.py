...
def _materials_count(path:pathlib.Path, counter:int, results:List[dict], _paths:List[pathlib.Path] = []):
...
def materials_count(path:pathlib.Path) -> List[dict]:
    results = []
    _paths = []
...
def check_materials_count(path:pathlib.Path):
